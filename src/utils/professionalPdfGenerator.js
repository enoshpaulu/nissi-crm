...
export const generateProfessionalQuotationPDF = async (quotation, lead, items) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      img.onload = () => {
        ...
        doc.addImage(img, 'JPEG', logoX, 18, logoWidth, logoHeight);
        resolve();
      };
      img.onerror = () => {
  ...
  // Right side - Date and Quote Number (MOVED MORE RIGHT)
  ...
  // Prepare table data with product images
  const tableDataPromises = items.map(async (item) => {
    let imageData = null;
  ...
  autoTable(doc, {
    startY: yPos,
    head: [[
      { content: 'MODEL', styles: { halign: 'left', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'DESCRIPTION', styles: { halign: 'left', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'IMAGE', styles: { halign: 'center', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'REQ. UNITS', styles: { halign: 'center', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'UNIT PRICE', styles: { halign: 'right', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'AMOUNT', styles: { halign: 'right', fillColor: [255, 102, 0], textColor: [255, 255, 255] } }
    ]],
    body: tableData,
    theme: 'grid',
    headStyles: {
    ...
    columnStyles: {
      0: { cellWidth: 32, fontStyle: 'bold', valign: 'top' },    // Model
      1: { cellWidth: 62, valign: 'top' },                        // Description
      2: { cellWidth: 22, halign: 'center', valign: 'middle' },  // Image
      3: { cellWidth: 20, halign: 'center', valign: 'middle' },  // Req Units
      4: { cellWidth: 28, halign: 'right', valign: 'middle' },   // Unit Price
      5: { cellWidth: 28, halign: 'right', valign: 'middle' }    // Amount (not Total Price)
    },
    alternateRowStyles: {
    ...
    didDrawCell: function(data) {
      if (data.column.index === 2 && data.cell.section === 'body') {
        const item = tableDataItems[data.row.index];
        if (item.image) {
          try {
            const imgWidth = 18;
            const imgHeight = 18;
  ...
  doc.rect(14, tableEndY, pageWidth - 28, 7, 'F');
  
  doc.setFontSize(9);
  doc.setTextColor(30);
  doc.setFont(undefined, 'bold');
  doc.text('SUB-TOTAL', pageWidth - 65, tableEndY + 4.5);
  
  const subtotal = items.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0);
  doc.text(`Rs. ${subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, pageWidth - 14, tableEndY + 4.5, { 
    align: 'right' 
  });
  
  // ========== GST BREAKDOWN & TOTAL ==========
  let totalY = tableEndY + 14;
  ...
  doc.text(`Rs. ${subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, rightX, totalY, { align: 'right' });
  ...
  doc.setFont(undefined, 'bold');
  doc.setTextColor(30);
  doc.text('THANK YOU AND ASSURING BEST SERVICE', pageWidth / 2, footerY, { align: 'center' });
  
  doc.setFontSize(8);
  doc.setFont(undefined, 'normal');
  doc.setTextColor(60);
  doc.text(`From M/S ${COMPANY.name}`, pageWidth / 2, footerY + 4.5, { align: 'center' });
  
  doc.setFontSize(7);
  doc.text(`${COMPANY.phone} | ${COMPANY.email}`, pageWidth / 2, footerY + 9, { align: 'center' });
  
  // System-generated disclaimer
  doc.setFontSize(7);
  doc.setTextColor(120);
  doc.setFont(undefined, 'italic');
  doc.text('This is a computer-generated quotation and does not require a signature.', pageWidth / 2, footerY + 14, { align: 'center' });
...
export const generateProfessionalInvoicePDF = async (invoice, lead, items) => {
  ...
  // Right side - Date and Invoice Number (MOVED MORE RIGHT)
  const rightMargin = 15;
  doc.setFontSize(9);
  doc.setTextColor(60);
  doc.setFont(undefined, 'bold');
  ...
  // ========== PRODUCTS TABLE - NO IMAGE, NO HSN ==========
  yPos = 72;
  ...
  autoTable(doc, {
    startY: yPos,
    head: [[
      { content: 'Sr', styles: { halign: 'center', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'MODEL', styles: { halign: 'left', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'DESCRIPTION', styles: { halign: 'left', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'REQ. UNITS', styles: { halign: 'center', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'RATE', styles: { halign: 'right', fillColor: [255, 102, 0], textColor: [255, 255, 255] } },
      { content: 'AMOUNT', styles: { halign: 'right', fillColor: [255, 102, 0], textColor: [255, 255, 255] } }
    ]],
    body: tableData,
    theme: 'grid',
    ...
    columnStyles: {
      0: { cellWidth: 10, halign: 'center', valign: 'middle' },  // Sr
      1: { cellWidth: 35, fontStyle: 'bold', valign: 'top' },    // Model
      2: { cellWidth: 70, valign: 'top' },                        // Description (WIDER)
      3: { cellWidth: 22, halign: 'center', valign: 'middle' },  // Req Units
      4: { cellWidth: 28, halign: 'right', valign: 'middle' },   // Rate
      5: { cellWidth: 28, halign: 'right', valign: 'middle' }    // Amount
    },
    alternateRowStyles: {
      fillColor: [250, 250, 250]
    }
  ...
  // ========== PAYMENT TERMS (NO T&C IN INVOICE) ==========
  ...
  doc.setTextColor(120);
  doc.setFont(undefined, 'italic');
  doc.text('This is a computer-generated invoice and is valid without signature.', pageWidth / 2, footerY + 14, { align: 'center' });
